...
bool accura::isPointInTriangle(const Eigen::Vector2d& point, const std::array<Eigen::Vector2d, 3>& trigon, double tolerance) // 2D
{
	...
	if (-precision < std::min(std::min(trigonR[0][0], trigonR[1][0]), trigonR[2][0]) ||
		precision > std::max(std::max(trigonR[0][0], trigonR[1][0]), trigonR[2][0]) ||
...
bool accura::isTwoSegmentsIntersect(const std::array<Vector2d, 2>& segmA, const std::array<Vector2d, 2>& segmB, double tolerance)
{
	...
	double precision = 0;
	precision = std::max(std::max(fabs(vecA[0]), fabs(vecA[1])), precision);
	precision = std::max(std::max(fabs(vecB[0]), fabs(vecB[1])), precision);
	precision = precision * tolerance;// assert(precision > tolerance);
    if (precision < std::min(segmB[0][0], segmB[1][0]) - std::max(segmA[0][0], segmA[1][0]) ||
		precision < std::min(segmA[0][0], segmA[1][0]) - std::max(segmB[0][0], segmB[1][0]) ||
        precision < std::min(segmB[0][1], segmB[1][1]) - std::max(segmA[0][1], segmA[1][1]) ||
		precision < std::min(segmA[0][1], segmA[1][1]) - std::max(segmB[0][1], segmB[1][1]))
		return false;

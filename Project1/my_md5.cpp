...
#define shift(x, n) (((x) << (n)) | ((x) >> (32-(n))))//右移的时候，高位一定要补零，而不是补充符号位
	...
	*/
	...
	unsigned int* addULL(const void* str, size_t len)
	{
		...
		{
			strByte[i >> 2] |= *((unsigned char*)str + i) << ((i % 4) * 8);//一个整数存储四个字节，i>>2表示i/4 一个unsigned int对应4个字节，保存4个字符信息
		}
	...
	string getMD5(const string& source)
	{
		...
		auto ita = changeHex(atemp);
		auto itb = changeHex(btemp);
		auto itc = changeHex(ctemp);
		auto itd = changeHex(dtemp);
...
{
	//para::C c;
	//getMD5(&c, sizeof(para::C));
	std::string a = "中文";
	std::vector<unsigned char> ucvec;
	ucvec.resize(a.size());
	memcpy(ucvec.data(), a.data(), a.size());
}
